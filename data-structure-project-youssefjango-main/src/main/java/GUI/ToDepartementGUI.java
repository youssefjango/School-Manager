...
/**
 * The English Depratment panel.
 *
 * @author YOUSSEF
 */
public class ToDepartementGUI extends javax.swing.JFrame {
     ...
     */
    public ToDepartementGUI(Departement departement) {
        this.departement = departement;
        if (!new java.io.File("Database\\" + this.departement.getDepartementName().replace('_', ' ') + ".txt").exists()) {
            DataConvertor.convertToFile(departement, "Database\\" + this.departement.getDepartementName().replace('_', ' ') + ".txt");
        }
        initComponents();
        nameLabels();
        startTables();
    ...
    private void initComponents() {
        ...
        depNameTextField = new javax.swing.JTextArea();
        ...
        depNameTextField.setBackground(new java.awt.Color(0, 0, 0));
        depNameTextField.setColumns(20);
        depNameTextField.setFont(new java.awt.Font("Cascadia Mono", 1, 36)); // NOI18N
        depNameTextField.setForeground(new java.awt.Color(255, 255, 255));
        depNameTextField.setRows(1);
        depNameTextField.setText("DepartmentName");
        depNameTextField.setBorder(new javax.swing.border.MatteBorder(null));
        depNameTextField.setCaretColor(new java.awt.Color(0, 0, 204));
        depNameTextField.setCursor(new java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR));
        jScrollPane5.setViewportView(depNameTextField);
    ...
    private void nameLabels() {
        depNameTextField.setText(this.departement.getDepartementName() + " Department");
        Departement d = DataConvertor.convertToObjects("Database\\" + this.departement.getDepartementName().replace('_', ' ') + ".txt");
        if (d.getDean() != null) {
    ...
    private void addStaffButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addStaffButtonActionPerformed
        try {
            ...
            Departement department = DataConvertor.convertToObjects("Database\\" + this.departement.getDepartementName().replace('_', ' ') + ".txt");
            ArrayList<Departement> departements = DataConvertor.convertAllDepartments();

            for (int j = 0; j < departements.size(); j++) {
                Departement departTemp = DataConvertor.convertToObjects("Database\\" + departements.get(j).getDepartementName().replace('_', ' ') + ".txt");
                if (departTemp.getStaffs() == null) {
                    continue;
                }
                for (int i = 0; i < departTemp.getStaffs().size(); i++) {
                    if (ID == departTemp.getStaffs().get(i).getID()) {
                        JOptionPane.showMessageDialog(null, ID + " exists in the " + departTemp.getDepartementName() + " Department as teacher.");
                        throw new IDInSameSchoolException();
                    }
                }
            }
            department.getStaffs().add(newStaff);
            DataConvertor.convertToFile(department, "Database\\" + this.departement.getDepartementName().replace('_', ' ') + ".txt");
            modelStaff = (DefaultTableModel) staffTable.getModel();
            modelTeacher = (DefaultTableModel) teacherTable.getModel();
    ...
    private void addTeacherButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addTeacherButtonActionPerformed
        try {
            ...
            for (int j = 0; j < departements.size(); j++) {
                ...
                Departement departTemp = DataConvertor.convertToObjects("Database\\" + departements.get(j).getDepartementName().replace('_', ' ') + ".txt");
                System.out.println(departTemp);
                if (departTemp.getTeachers() == null) {
                    continue;
                }
                for (int i = 0; i < departTemp.getTeachers().size(); i++) {
                    if (ID == departTemp.getTeachers().get(i).getID()) {
                        JOptionPane.showMessageDialog(null, ID + " exists in the " + departTemp.getDepartementName() + " Department as teacher.");
                        throw new IDInSameSchoolException();
                    }
                }
            ...
            if (department.getTeachers().isEmpty()) {
                department.setDean(new Dean(teacher.getAge(), teacher.getName(), teacher.getIsMale(), teacher.getHoursWorked(),
                        teacher.getEmail(), teacher.getYearsOfExp(), teacher.getID(), teacher.getSpecialty(),
                        teacher.getDegree(), teacher.getIsPart_Time(), 0));
            }
    ...
    private void changeDeanButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_changeDeanButtonActionPerformed
        try {
            ...
            if (t == null) {
                ...
                DataConvertor.convertToFile(d, "Database\\" + this.departement.getDepartementName().replace('_', ' ') + ".txt");
            }
        } catch (Exception e) {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ToDepartementGUI.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ToDepartementGUI().setVisible(true);
            }
        });
    }
    ...
    private javax.swing.JTextArea depNameTextField;
